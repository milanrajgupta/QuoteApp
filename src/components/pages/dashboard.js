...
const Dashboard = () => {
  ...
  const getAllUserQuotes = async () => {
    try {
        setLoading(true)
      const response = await fetch('http://localhost:8000/quote/all-user-quote', {
        method: 'POST',
        headers: {
      ...
      const sortedQuote=data.data.sort((a,b)=>b-a)
      setQuotes(sortedQuote)
  ...
  return (
    ...
    loading?(<div><Loader/></div>):(
        <div className=' min-h-screen  flex items-center justify-center '>
         <div className="grid grid-cols-2 w-[1000px] bg-[#13515c] justify-center mx-auto  text-white mt-[100px] gap-4 p-4">
